...
assert_eq "$(tier_for 2 merge working)"  "5" "working = in progress (overrides merge)"
assert_eq "$(tier_for 3 none working)"   "5" "working = in progress"
assert_eq "$(tier_for 2 merge idle)"     "2" "high ready-to-merge = ready to merge (not act now)"
