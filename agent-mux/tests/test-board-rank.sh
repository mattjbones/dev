...
# Expected: tier1 (urgent-blocked), tier4 (med-blocked), tier5 (high-review by rank, working), tier6 (low-idle)
assert_eq "$order" "urgent-blocked,med-blocked,high-review,working,low-idle," "ranked order"
tiers="$(printf '%s' "$out" | jq -r '.[].tier' | tr '\n' ',')"
assert_eq "$tiers" "1,4,5,5,6," "tiers"
...
tie='[{"key":"older","priority":3,"reviewState":"none","attention":"working","lastFocus":50},
      {"key":"newer","priority":3,"reviewState":"none","attention":"working","lastFocus":99}]'
